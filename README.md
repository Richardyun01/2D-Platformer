...
- 플레이어의 움직임을 따라갈 수 있도록 카메라 조작
  - 기존에 존재하였던 카메라를 기반으로 함
  - 각 Scene의 카메라에 플레이어를 연결하면 됨
  
- 각 스테이지에 해당하는 Scene 다수 준비
  - 필요한 Scene: 스테이지 3 x 레벨 3 = 총 9개
...
- 플레이어의 특성
  - 체력
    - 플레이어의 기본 체력: 100
    - 체력이 0 이하가 되면 게임 오버
    - 체력 관련 공식이 기획되었으나 사용되지 않음
    - 게임 중 UI의 체력바, 일시 정지 UI의 스탯과 연결 
    - 추후 다른 시스템이나 오브젝트와 연결될 여지가 있도록 함
  - 공격
    - 좌클릭 시 투사체 발사
    - 투사체 관련 수치: 수명, 발사 주기, 피해량, 탄환 프리팹
    - 탄환은 적과 접촉할 시 피해를 주고 삭제되어야 함
    - 추후 다른 시스템이나 오브젝트와 연결될 여지가 있도록 함
  - 이동
    - 기존에 존재했던 플레이어 물리 엔진을 기반으로 함
...
- 맵에 배치되어야 하는 적 제작
  ...
  - 적 관련 수치: 체력, 공격력, 추적 거리, 사격 거리, 넉백 수치, 귀환 거리
...
## UI 개발
- 인게임 외부 UI
  - 필요한 Scene: 타이틀 1 + 메인 1 + 각 스테이지 3 + 클리어 1 = 6
  - 타이틀
